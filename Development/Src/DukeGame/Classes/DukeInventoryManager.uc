// DukeInventoryManager.uc
//

class DukeInventoryManager extends InventoryManager
	config(Game);
...
{
	var	int				Amount;
	var class<DukeWeapon> WeaponClass;
};
...
var DukeWeapon PendingSwitchWeapon;
...
{
	local DukeInventory	Inv;

	ForEach InventoryActors(class'DukeInventory', Inv)
	{
 ...
 */
simulated function GetWeaponList(out array<DukeWeapon> WeaponList, optional bool bFilter, optional int GroupFilter, optional bool bNoEmpty)
{
	local DukeWeapon Weap;
	local int i;

	ForEach InventoryActors( class'DukeWeapon', Weap )
	{
		if ( (!bFilter) && ( !bNoEmpty || Weap.HasAnyAmmo()) )
		{
...
{
	local DukeWeapon CurrentWeapon;
	local array<DukeWeapon> WeaponList;
	local int NewIndex;
	...
	CurrentWeapon = DukeWeapon(PendingWeapon);
	if (CurrentWeapon == None)
	{
		CurrentWeapon = DukeWeapon(Instigator.Weapon);
	}
...
{
	if ( DukeWeapon(Pawn(Owner).Weapon) != None && DukeWeapon(Pawn(Owner).Weapon).DoOverridePrevWeapon() )
		return;
...
{
	if ( DukeWeapon(Pawn(Owner).Weapon) != None && DukeWeapon(Pawn(Owner).Weapon).DoOverrideNextWeapon() )
		return;
...
function AllAmmo(optional bool bAmmoForSuperWeapons)
{
	...
	for( Inv=InventoryChain; Inv!=None; Inv=Inv.Inventory )
		if ( (DukeWeapon(Inv)!=None) && (bAmmoForSuperWeapons) )
			DukeWeapon(Inv).Loaded(true);
}
...
{
	local DukeWeapon PrevWeapon, CurrentPending;
	local DukePawn UTP;
		...
		(CurrentPending == None || CurrentPending.AllowSwitchTo(DesiredWeapon)) )
	{
		...
		else
		{
			if ( Instigator.IsHumanControlled() && Instigator.IsLocallyControlled() )
			{
				...
				if ( DukeWeapon(Instigator.Weapon) != None )
				{
					DukeWeapon(Instigator.Weapon).PreloadTextures(false);
				}
				if ( PendingWeapon != None )
				{
					DukeWeapon(PendingWeapon).PreloadTextures(false);
				}
	 			DukeWeapon(DesiredWeapon).PreloadTextures(true);
			}
...
simulated function ClientWeaponSet(Weapon NewWeapon, bool bOptionalSet, optional bool bDoNotActivate)
{
	...
	if ( OldWeapon.IsFiring() || OldWeapon.DenyClientWeaponSet() && (DukeWeapon(NewWeapon) != None) )
	{
		NewWeapon.GotoState('Inactive');
		RetrySwitchTo(DukeWeapon(NewWeapon));
			return;
		}
...
simulated function RetrySwitchTo(DukeWeapon NewWeapon)
{
...
simulated function CheckSwitchTo(DukeWeapon NewWeapon)
{
	if ( DukeWeapon(Instigator.Weapon) == None ||
			( Instigator != None && PlayerController(Instigator.Controller) != None &&
				DukeWeapon(Instigator.Weapon).ShouldSwitchTo(NewWeapon) ) )
	{
...
simulated function bool AddInventory( Inventory NewItem, optional bool bDoNotActivate )
{
	...
	if (Role == ROLE_Authority)
	{
		...
		if (bResult && DukeWeapon(NewItem) != None)
		{
			...
			for (i=0;i<AmmoStorage.Length;i++)
			{
				...
				{
					DukeWeapon(NewItem).AddAmmo(AmmoStorage[i].Amount);
			...
			if (!bDoNotActivate)
			{
				CheckSwitchTo(DukeWeapon(NewItem));
			}
		}
	}
...
function bool NeedsAmmo(class<DukeWeapon> TestWeapon)
{
	local array<DukeWeapon> WeaponList;
	local int i;
...
function AddAmmoToWeapon(int AmountToAdd, class<DukeWeapon> WeaponClassToAddTo)
{
	local array<DukeWeapon> WeaponList;
	local int i;
...
{
	local DukeWeapon Wep;
	local DukePawn UTP;

	PreviousWeapon = Instigator.Weapon;
	Super.ChangedWeapon();

	Wep = DukeWeapon(Instigator.Weapon);
